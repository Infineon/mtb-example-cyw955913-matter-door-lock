...
APPNAME=mtb-example-cyw955913-matter-door-lock
...
# Add defines to enable the touch LCD display and for LVGL library
#DEFINES+=ENABLE_UI LV_CONF_INCLUDE_SIMPLE
...
# This pulls in the source files for the various clusters associated with the
# selected example application.  (See the mtb_matter_data_model.mk file.)
MATTER_EXAMPLE_APP = lock-app
MATTER_PLATFORM = cat5

# Remove this if the application uses the ota-update library
COMPONENTS+=MATTER_OTA_UPDATE

# Put this code in RAM to ensure that there are no communication failure due to code
# execution latency.
PLACE_COMPONENT_IN_SRAM+=$(SEARCH_wifi-host-driver)
