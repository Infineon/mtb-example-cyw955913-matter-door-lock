...
cy_timer_t sLockTimer;

using namespace ::chip::DeviceLayer::Internal;
using namespace CYW955913DoorLock::LockInitParams;
...
CHIP_ERROR LockManager::Init(chip::app::DataModel::Nullable<chip::app::Clusters::DoorLock::DlLockState> state, LockParam lockParam)
{
    ...
    // Create sw timer for lock timer.
    if (cy_rtos_timer_init(&sLockTimer, CY_TIMER_TYPE_ONCE,
                           TimerEventHandler, (cy_timer_callback_arg_t)this) != CY_RSLT_SUCCESS)
    {
        INF_LOG("sLockTimer timer create failed");
...
{
    size_t outLen;
    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_LockUser, reinterpret_cast<uint8_t *>(&mLockUsers),
                                 sizeof(EmberAfPluginDoorLockUserInfo) * MATTER_ARRAY_SIZE(mLockUsers), outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_Credential, reinterpret_cast<uint8_t *>(&mLockCredentials),
                                 sizeof(EmberAfPluginDoorLockCredentialInfo) * kMaxCredentials * kNumCredentialTypes, outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_LockUserName, reinterpret_cast<uint8_t *>(mUserNames), sizeof(mUserNames),
                                 outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_CredentialData, reinterpret_cast<uint8_t *>(mCredentialData),
                                 sizeof(mCredentialData), outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_UserCredentials, reinterpret_cast<uint8_t *>(mCredentials),
                                 sizeof(CredentialStruct) * LockParams.numberOfUsers * kMaxCredentials, outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_WeekDaySchedules, reinterpret_cast<uint8_t *>(mWeekdaySchedule),
                                 sizeof(EmberAfPluginDoorLockWeekDaySchedule) * LockParams.numberOfWeekdaySchedulesPerUser *
                                     LockParams.numberOfUsers,
                                 outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_YearDaySchedules, reinterpret_cast<uint8_t *>(mYeardaySchedule),
                                 sizeof(EmberAfPluginDoorLockYearDaySchedule) * LockParams.numberOfYeardaySchedulesPerUser *
                                     LockParams.numberOfUsers,
                                 outLen);

    CYW955913Config::ReadConfigValueBin(CYW955913Config::kConfigKey_HolidaySchedules, reinterpret_cast<uint8_t *>(&(mHolidaySchedule)),
...
void LockManager::StartTimer(uint32_t aTimeoutMs)
{
    ...
    cy_rtos_timer_is_running(&sLockTimer, &state);
    if (state)
    {
        INF_LOG("app timer already started!");
        CancelTimer();
    }

    // timer is not active, change its period to required value (== restart).
    if (cy_rtos_timer_start(&sLockTimer, aTimeoutMs) != CY_RSLT_SUCCESS)
    {
        INF_LOG("sLockTimer timer start() failed");
...
{
    if (cy_rtos_timer_stop(&sLockTimer) != CY_RSLT_SUCCESS)
    {
        INF_LOG("sLockTimer stop() failed");
        appError(APP_ERROR_STOP_TIMER_FAILED);
    }
}

void LockManager::TimerEventHandler(cy_timer_callback_arg_t arg)
{
    ...
    event.TimerEvent.Context = (void *) arg;
                          ...
                          CredentialRuleEnum credentialRule, const CredentialStruct * credentials, size_t totalCredentials)
{
    ...
    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_LockUser, reinterpret_cast<const uint8_t *>(&mLockUsers),
                                  sizeof(EmberAfPluginDoorLockUserInfo) * LockParams.numberOfUsers);

    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_UserCredentials, reinterpret_cast<const uint8_t *>(mCredentials),
                                  sizeof(CredentialStruct) * LockParams.numberOfUsers * kMaxCredentials);

    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_LockUserName, reinterpret_cast<const uint8_t *>(mUserNames),
                                  sizeof(mUserNames));
                                ...
                                const chip::ByteSpan & credentialData)
{
    ...
    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_Credential, reinterpret_cast<const uint8_t *>(&mLockCredentials),
                                  sizeof(EmberAfPluginDoorLockCredentialInfo) * kMaxCredentials * kNumCredentialTypes);

    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_CredentialData, reinterpret_cast<const uint8_t *>(&mCredentialData),
                                         ...
                                         uint8_t endHour, uint8_t endMinute)
{
    ...
    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_WeekDaySchedules, reinterpret_cast<const uint8_t *>(mWeekdaySchedule),
                                  sizeof(EmberAfPluginDoorLockWeekDaySchedule) * LockParams.numberOfWeekdaySchedulesPerUser *
                                      LockParams.numberOfUsers);
                                         ...
                                         DlScheduleStatus status, uint32_t localStartTime, uint32_t localEndTime)
{
    ...
    // Save schedule information in NVM flash
    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_YearDaySchedules, reinterpret_cast<const uint8_t *>(mYeardaySchedule),
                                  sizeof(EmberAfPluginDoorLockYearDaySchedule) * LockParams.numberOfYeardaySchedulesPerUser *
                                      LockParams.numberOfUsers);
                                         ...
                                         uint32_t localStartTime, uint32_t localEndTime, OperatingModeEnum operatingMode)
{
    ...
    // Save schedule information in NVM flash
    CYW955913Config::WriteConfigValueBin(CYW955913Config::kConfigKey_HolidaySchedules, reinterpret_cast<const uint8_t *>(&(mHolidaySchedule)),
