...
#include <platform/Infineon/cat5/NetworkCommissioningDriver.h>
#include <platform/Infineon/cat5/FactoryDataProvider.h>
...
#define APP_TASK_PRIORITY (CY_RTOS_PRIORITY_NORMAL)
...
cy_timer_t sFunctionTimer; // app sw timer.
...
uint64_t appStack[(APP_TASK_STACK_SIZE + sizeof(uint64_t) - 1) / sizeof(uint64_t)];
...
static chip::DeviceLayer::FactoryDataProvider mFactoryDataProvider;

namespace {
app::Clusters::NetworkCommissioning::Instance
    sWiFiNetworkCommissioningInstance(0 /* Endpoint Id */, &(NetworkCommissioning::CYW955913WiFiDriver::GetInstance()));
} // namespace
...
CHIP_ERROR AppTask::StartAppTask()
{
    ...
    result = cy_rtos_queue_init(&sAppEventQueue, APP_EVENT_QUEUE_SIZE, sizeof(AppEvent));
    if (result != CY_RSLT_SUCCESS)
    {
        INF_LOG("Failed to allocate app event queue");
        appError(APP_ERROR_EVENT_QUEUE_FAILED);
    }
    // Start App task.
    result = cy_rtos_thread_create(&sAppTaskHandle, AppTaskMain,
                                   APP_TASK_NAME, appStack, APP_TASK_STACK_SIZE,
...
void AppTask::lockMgr_Init()
{
    ...
    INF_LOG("Current Software Version: %d", CHIP_DEVICE_CONFIG_DEVICE_SOFTWARE_VERSION);
    // Initial lock state
    ...
    // Create a sw timer for Function Selection.
    if (cy_rtos_timer_init(&sFunctionTimer, CY_TIMER_TYPE_ONCE,
                           TimerEventHandler, (cy_timer_callback_arg_t)this) != CY_RSLT_SUCCESS)
    {
        INF_LOG("funct timer create failed");
...
void AppTask::Init()
{
    ...
    if (rc != 0)
    {
        INF_LOG("boot_set_confirmed failed");
...
void AppTask::AppTaskMain(cy_thread_arg_t arg)
{
    AppEvent event;

    sAppTask.Init();
    INF_LOG("App Task started");

    while (true)
    {
        cy_rslt_t eventReceived = cy_rtos_queue_get(&sAppEventQueue, &event, CY_RTOS_NEVER_TIMEOUT);
        if (eventReceived == CY_RSLT_SUCCESS)
        {
...
void AppTask::LockActionEventHandler(AppEvent * event)
{
    ...
    case AppEvent::kEventType_Button: {
        ...
        INF_LOG("%s [Action: %d]", __FUNCTION__, event->ButtonEvent.Action);

        if (event->ButtonEvent.Action == APP_BUTTON_LONG_PRESS)
        {
            INF_LOG("Sending a lock jammed event");
...
void AppTask::TimerEventHandler(cy_timer_callback_arg_t arg)
{
    AppEvent event;
...
void AppTask::FunctionHandler(AppEvent * event)
{
    ...
    if (event->ButtonEvent.Action == APP_BUTTON_LONG_PRESS)
    {
        ...
        {
            INF_LOG("Factory Reset Triggered. Press button again within %us to cancel.", FACTORY_RESET_CANCEL_WINDOW_TIMEOUT / 1000);
        ...
        else if (sAppTask.mFunctionTimerActive && sAppTask.mFunction == Function::kFactoryReset)
        {
            ...
            INF_LOG("Factory Reset has been Canceled");
        }
    }
#if ENABLE_BUTTON_LOCK_UNLOCK
    else
    {
        LockManager::Action_t action;
        int32_t actor;
...
{
    if (cy_rtos_timer_stop(&sFunctionTimer) != CY_RSLT_SUCCESS)
    {
        INF_LOG("app timer stop() failed");
...
void AppTask::StartTimer(uint32_t aTimeoutInMs)
{
    ...
    cy_rtos_timer_is_running(&sFunctionTimer, &state);
    if (state)
    {
    ...
    if (cy_rtos_timer_start(&sFunctionTimer, aTimeoutInMs) != CY_RSLT_SUCCESS)
    {
        INF_LOG("app timer start() failed");
...
void AppTask::ActionInitiated(LockManager::Action_t aAction, int32_t aActor)
{
    ...
    {
        INF_LOG("Lock Action has been initiated");
    }
    else if (aAction == LockManager::UNLOCK_ACTION)
    {
        INF_LOG("Unlock Action has been initiated");
    }
...
{
    if (cy_rtos_queue_put(&sAppEventQueue, event, 0) != CY_RSLT_SUCCESS)
    {
        INF_LOG("Failed to post event to app task event queue");
    }
}
...
void AppTask::DispatchEvent(AppEvent * event)
{
    ...
    {
        INF_LOG("Event received with no handler. Dropping event.");
    }
}
...
void AppTask::UpdateCluster(intptr_t context)
{
    ...
    {
        INF_LOG("ERR: updating lock state %x", to_underlying(status));
    }
}
